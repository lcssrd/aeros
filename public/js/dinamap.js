/**
 * Logic for the Aeros Dinamap A100 Vital Signs Monitor.
 * Fully synchronized with the instructor pilot and feature-parity with monitor.js.
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Display room code & update switcher link to monitor.html
  const displayRoomEl = document.getElementById('display-room');
  if (displayRoomEl) {
    displayRoomEl.textContent = roomCode;
  }
  const btnSwitchMonitor = document.getElementById('btn-switch-monitor');
  if (btnSwitchMonitor) {
    btnSwitchMonitor.href = `monitor.html?room=${encodeURIComponent(roomCode)}`;
  }
  ...
  function performMeasurement() {
    ...
    setTimeout(() => {
      ...
      if (!isContinuous) {
        els.btnOneshot?.classList.remove('active-btn');
  ...
  function stopCycleTension() {
    ...
    els.btnContinue?.classList.remove('active-mode');
  ...
  function handleOneShotNIBP() {
    ...
    stopCycleTension();
    els.btnOneshot?.classList.add('active-btn');
    if (els.btnStop) els.btnStop.disabled = false;
    performMeasurement();
  }
  ...
  function handleAutoCycleNIBP() {
    ...
    isContinuous = true;
    if (els.btnStop) els.btnStop.disabled = false;
    els.btnContinue?.classList.add('active-mode');
    els.btnCycle?.classList.add('active-mode');
    performMeasurement();
    startCycleCountdown(SIMULATION_CONFIG.NIBP_CYCLE_INTERVAL_SEC);
  }

  if (els.btnOneshot) els.btnOneshot.addEventListener('click', handleOneShotNIBP);
  if (els.btnInflate) {
  ...
  if (els.btnContinue) els.btnContinue.addEventListener('click', handleAutoCycleNIBP);
  if (els.btnCycle) {
  ...
  if (els.btnStop) {
    els.btnStop.addEventListener('click', handleStopAll);
  }
  ...
  if (els.btnPlus) {
    els.btnPlus.addEventListener('click', () => {
      // Local value or threshold stepping if needed
